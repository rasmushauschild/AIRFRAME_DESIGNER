...
def analyze(path):
 ...
 rx=-.3*np.cos(psi)*np.cos(theta)+.3*(np.cos(psi)*np.sin(theta)*np.cos(phi)+np.sin(psi)*np.sin(phi))
 ry=-.3*np.sin(psi)*np.cos(theta)+.3*(np.sin(psi)*np.sin(theta)*np.cos(phi)-np.cos(psi)*np.sin(phi))
